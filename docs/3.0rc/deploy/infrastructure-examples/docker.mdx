...
2.  These images are tagged with the full description, e.g. `prefect:3.0.0-python3.12` and `prefect:3.0.0-python3.12-conda`.
3.  For users that want more specific pins, these images are also tagged with the SHA of the git commit of the release, e.g. `sha-88a7ff17a3435ec33c95c0323b8f05d7b9f3f6d2-python3.12`
4.  For users that want to be on the latest `3.0.x` release, receiving patch updates, we update a tag without the patch version to this release, e.g. `prefect.3.0-python3.12`.
5.  For users that want to be on the latest `3.x.y` release, receiving minor version updates, we update a tag without the minor or patch version to this release, e.g. `prefect.3-python3.12`
...
Standard Python images are based on the official Python `slim` images, e.g. `python:3.12-slim`.
